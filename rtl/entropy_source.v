...
// Module Name ;, entropy_source.v—Multi-oscillator jitter sampler
//=============================================================================
...
module entropy_source #(
    parameter DECIM_BITS = 8       //sample once every 2^DECIM_BITS clocks
                                   //8→Every 256 clocks will give around ~390 k samples/s
)(
    input  wire clk,
    input  wire rst,
    output reg  raw_bit,
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (decim_cnt == {DECIM_BITS{1'b1}}) begin   //all ones will rollover
                raw_bit   <= sync_safe;
                raw_valid <= 1'b1;
            end else begin
                raw_valid <= 1'b0;
